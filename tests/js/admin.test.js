...
describe("admin.js tests with Vitest", () => {
    ...
    describe("initialisation (synchronous, no DOMContentLoaded)", () => {
        ...
        it("should warn and not process when DjangoFormAlpine has no resolvers", () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                expect.stringContaining(
                    "DjangoFormAlpine or its resolvers not found",
                ),
            );
            warnSpy.mockRestore();
        });
        ...
        it("should merge custom resolvers when useAdminResolvers is true", () => {
            ...
            expect(window.DjangoFormAlpine.resolvers["form-row"]).toBeDefined();
            expect(window.DjangoFormAlpine.resolvers["custom"]).toBe(
                customResolver,
            );
        });
    ...
    describe("applyPrefixedDirectivesToContainer with __row_prefix__", () => {
        ...
        it("should replace __row_prefix__ with the ID of the closest .inline-related container", () => {
            ...
            const el = document.createElement("input");
            el.setAttribute(
                "x-field-container-model",
                "__row_prefix__otherField",
            );
            wrapper.appendChild(el);
            ...
            expect(container.getAttribute("x-model")).toBe(
                "items_0_otherField",
            );
        });
    ...
    describe("formset:added — dynamic inline rows", () => {
        ...
        it("should call Alpine.initTree on the new row when Alpine is present", () => {
            ...
            window.prepareAlpineBeforeLoad({});
            row.dispatchEvent(
                new CustomEvent("formset:added", { bubbles: true }),
            );
    ...
    describe("prepareAlpineBeforeLoad", () => {
        it("should initialize x-model and update x-data in the form", () => {
            ...
            form.setAttribute(
                "x-data",
                JSON.stringify({ existing_data: "123" }),
            );
        ...
        it("should warn if both x-add-model-data and x-model are present", () => {
            ...
            expect(spy).toHaveBeenCalledWith(
                expect.stringContaining(
                    'has both "x-add-model-data" and "x-model"',
                ),
            );
        ...
        it("should not overwrite existing x-data keys", () => {
            ...
            form.setAttribute(
                "x-data",
                JSON.stringify({ kept_field: "old-value" }),
            );
        ...
        it("should return early and not throw if input is completely outside a form", () => {
            ...
            expect(() => {
                window.prepareAlpineBeforeLoad(
                    window.djangoAdminAlpineResolvers,
                );
            }).not.toThrow();
        });
